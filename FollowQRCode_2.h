...
public:
	Pose m_CurrentPose;
	Pose m_PreviousPose;
	Pose m_VecIMRToQRCodeAfter;
	Pose m_VecIMRToQRCodeBefore;
...
public:
	CIMR_Trajectory()
	{
		m_CurrentPose.m_Point.x = -700.0;
		m_CurrentPose.m_Point.y = 160.0;
		m_CurrentPose.m_dOrientation = 0.0;
	...
	bool InsideFOV(Pose QRCodeWorldFramePose)
	{
		...
		m_VecIMRToQRCodeAfter.m_dOrientation = QRCodeWorldFramePose.m_dOrientation - m_CurrentPose.m_dOrientation;
		...
		if (CGeometryobj.PointInsideTriangle(TrianglePoints, QRCodeWorldFramePoint) &&
			fabs(m_VecIMRToQRCodeAfter.m_dOrientation) < QRCode_ThresholdAngle)
		{
			m_fTimeSpentOutsideFOV = 0;
			return true;
		}
	...
	{		
		//printf("m_CurrentPose.m_Point.x %f\n", m_CurrentPose.m_Point.x);
		//printf("m_CurrentPose.m_Point.y %f\n", m_CurrentPose.m_Point.y);
		//printf("m_CurrentPose.m_dOrientation %f\n", m_CurrentPose.m_dOrientation);
		//printf("QRCodeWorldFramePose.m_Point.x %f\n", QRCodeWorldFramePose.m_Point.x);
		//printf("QRCodeWorldFramePose.m_Point.y %f\n", QRCodeWorldFramePose.m_Point.y);
		m_fDeltaTheta = CGeometryobj.CalculateVectorIMRTOQRCode(QRCodeWorldFramePose, m_CurrentPose, m_VecIMRToQRCodeAfter);
		m_fDistanceIMRToQRCode = m_VecIMRToQRCodeAfter.m_Point.y;
		//printf("m_VecIMRToQRCodeAfter.m_Point.x %f\n", m_VecIMRToQRCodeAfter.m_Point.x);
		//printf("m_VecIMRToQRCodeAfter.m_Point.y %f\n", m_VecIMRToQRCodeAfter.m_Point.y);
		//printf("m_VecIMRToQRCodeAfter.m_dOrientation %f\n", m_VecIMRToQRCodeAfter.m_dOrientation);
		//_getch();
		//m_VecIMRToQRCodeAfter.m_Point.x = QRCodeWorldFramePose.x - m_CurrentPose.m_Point.x;
		//m_VecIMRToQRCodeAfter.m_Point.y = QRCodeWorldFramePose.y - m_CurrentPose.m_Point.y;
		//m_VecIMRToQRCodeAfter.m_dOrientation = 0.0;
	}
	void CalculateError(Pose QRCodeWorldFramePose)
	{
		m_VecIMRToQRCodeBefore = m_VecIMRToQRCodeAfter;
		Point2D QRCodeWorldFramePoint = QRCodeWorldFramePose.m_Point;
		...
		if (!InRange_FOV && m_bStartFollowing)
		{
			...
			//cout << "m_mStateTransition" << CKalmanFilterobj.m_mStateTransition << endl;
			//cout << "m_mX_Posteriori" << CKalmanFilterobj.m_mX_Posteriori << endl;
			//cout << "m_mU" << CKalmanFilterobj.m_mU << endl;
			//cout << "m_mX_Priori" << CKalmanFilterobj.m_mX_Priori << endl;
			CalculateCameraVector(QRCodeWorldFramePose);
			m_VecIMRToQRCodeAfter.m_dOrientation = QRCodeWorldFramePose.m_dOrientation - m_CurrentPose.m_dOrientation;
		}
		
		printf("vector (X, Y, theta): (%f,%f,%f)\n", m_VecIMRToQRCodeAfter.m_Point.x, m_VecIMRToQRCodeAfter.m_Point.y, m_VecIMRToQRCodeAfter.m_dOrientation);
		//printf("IMR (X, Y, theta): (%f,%f,%f)\n", m_CurrentPose.m_Point.x, m_CurrentPose.m_Point.y, m_CurrentPose.m_dOrientation);
		//printf("QRCode (X, Y): (%f,%f)\n", QRCodeObject->m_CurrentPose.m_Point.x, QRCodeObject->m_CurrentPose.m_Point.y);
		
		//m_fDeltaTheta = CGeometryobj.DetermineOrientationNeeded(m_CurrentPose.m_Point, QRCodeWorldFramePoint, m_CurrentPose.m_dOrientation);
		//printf("m_fDeltaTheta %f\n", m_fDeltaTheta);
...
public:
	...
	void Initialize()
	{
		...
		const char* filename = "QRCodeTrajectory_Data/Data_LShape.csv";
		...
		// PID_AngularSpeed
		PID_AngularSpeed->m_dKP = 17.5; //17.5 0.7 2.0
		...
		while (CIMRObject->m_fDistanceIMRToQRCode >= CIMRObject->m_fMaintainDistance + 5)
		{
			...
			fprintf(auf, "%f,%f,%f,%f, %f,%f,%d, %f,%f,%f,%f,%f,%f,%f,%f\n", m_fTime, CIMRObject->m_fDistanceIMRToQRCode, CIMRObject->m_fDeltaTheta,
				CIMRObject->m_VecIMRToQRCodeAfter.m_dOrientation,
				CIMRObject->m_VecIMRToQRCodeAfter.m_Point.x,
				CIMRObject->m_VecIMRToQRCodeAfter.m_Point.y,
		...
		//printf("QRCodeTrajectoryTime[SimualtionCount] %f\n", QRCodeTrajectoryTime[SimualtionCount]);
		//printf("QRCodeTrajectoryTime[SimualtionCount - 1] %f\n", QRCodeTrajectoryTime[SimualtionCount - 1]);
		//printf("q_dot %f\n", q_dot);
		//printf("qori_dot %f\n", qori_dot);
		//_getch();
